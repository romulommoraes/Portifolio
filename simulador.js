...
function init() {
    ...
    var selecao12 = 0;
    var selecao22 = 0;
    var numIndividuos = constrain(parseInt(document.getElementById('numIndividuos1').value), 2, 1000);
    limitPop = constrain(parseInt(document.getElementById('MaxPop').value), 2, 1000);
    fP = constrain(parseFloat(document.getElementById('freqP').value), 0, 1);
    selecao11 = constrain(parseInt(document.getElementById('selxAA').value), 0, 100);
    selecao12 = constrain(parseInt(document.getElementById('selxAa').value), 0, 100);
    selecao22 = constrain(parseInt(document.getElementById('selxaa').value), 0, 100);
    if (isNaN(numIndividuos) === true) {
        console.log(numIndividuos);
        numIndividuos = 100;
    }
    if (isNaN(limitPop) === true) {
        limitPop = 300;
    }
    if (isNaN(fP) === true) {
        fP = 0.5;
    }
    if (isNaN(selecao11) === true) {
        selecao11 = 0;
    }
    if (isNaN(selecao12) === true) {
        selecao12 = 0;
    }
    if (isNaN(selecao22) === true) {
        selecao22 = 0;
    }
    // variáveis básicas
    let larguraSet = 600;
    ...
    function startTime() {
        ...
        graphFP.splice(0, graphFP.length);
        graphFQ.splice(0, graphFQ.length);
        graphTime.splice(0, graphTime.length);
        graphAA.splice(0, graphAA.length);
        graphAa.splice(0, graphAa.length);
        graphaa.splice(0, graphaa.length);
    ...
    class Individuo {
        constructor() {
            ...
            this.size = 5;
            this.idade = 0;
            this.tempoExtra = round(random(-10, 11), 1); //determina o número de gerações que o animal vive a menos ou a mais que o valor base.
        }
        ...
        /* função pra gerar seleção natural por área
    ...
    function startSim() {
        ...
        function selecaoNatural() {
            for (var i = 0; i < saudaveis.length; i++) {
                ...
                if (dominancias === 11 && probMorrer < selecao11) {
                    saudaveis.splice(i, 1);
                    homozigotos11--;
                };
                if (dominancias === 12 && probMorrer < selecao12) {
                    saudaveis.splice(i, 1);
                    heterozigotos--;
                };
                if (dominancias === 22 && probMorrer < selecao22) {
                    saudaveis.splice(i, 1);
                    homozigotos22--;
                };
            }
        ...
        function morteIdade() {
            ...
            for (var i = 0; i < saudaveis.length; i++) {
                ...
                if (idadeInd >= 12 + tempoIncr) {
                    ...
                    saudaveis.splice(i, 1);
                    if (dominancias === 11) {
                        homozigotos11--; /*console.log(homozigotos11 + "pau");*/
                    }
                    if (dominancias === 12) {
                        heterozigotos--;
                    }
            ...
            setInterval(function () {
                ...
                time++;
                graphTime.push(time);
                graphFP.push(round(fP, 3));
                graphFQ.push(round(1 - fP, 3));
                graphAA.push(homozigotos11);
                graphAa.push(heterozigotos);
                graphaa.push(homozigotos22);
                //console.log(graphY);

                //envelhecimento();
                if (selecao11 != 0 || selecao12 != 0 || selecao22 != 0) {
                    selecaoNatural();
                }
            }
            }, 1000);
        ...
        draw = function () {
            ...
            if (time > 5 && fP === 1 || time > 5 && fP === 0 || saudaveis.length === 0) {parar();} 
            ...
            var arraySize = saudaveis.length;
